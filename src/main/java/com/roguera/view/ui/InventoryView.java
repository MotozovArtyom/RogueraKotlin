...
public class InventoryView implements IViewBlock {
	...
	public void useItem(int index) {
		if (!quickEqpElements.isEmpty()) {
			...
			getElement(indexOfSlot).ifPresentOrElse(this::elementActionRun, () -> ViewObjects.logView.putLog("Quick slot is empty!"));
		} else {
			ViewObjects.logView.putLog("Quick slots is empty!");
		}
		Draw.call(this);
	}
	...
	private final Runnable accessItem = () -> {
		...
		if (eq instanceof Usable) {
			...
			ViewObjects.logView.playerAction("have used a " + eq.model.getModelColorName() + "!");
