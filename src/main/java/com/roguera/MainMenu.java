...
public class MainMenu {
	...
	private static void startMainMenuRP() {
		log.info("[DISCORD_RP]Start main menu presence");
		...
		log.info("[DISCORD_RP] RP set");
	}

	public static void start(int code) {
		try {
			log.info("[MAIN_MENU]Loading main menu");
	...
	private static void ConstructNewGamePanel() {
		...
		newGamePanel.addComponent(new Button("Start game", () -> {
			...
			if (nickname.getText().length() > 0) {
				log.info("[MAIN_MENU] get input nickname: " + nickname.getText());
				try {
					if (Roguera.INSTANCE.isOnline() && !ServerRequests.isNickNameAvailable(nickname.getText())) {
				...
				log.info("[MAIN_MENU] Set nickname " + Dungeon.player.getPlayerData().getPlayerName());
			} else {
				log.info("[MAIN_MENU] set random nickname");
			}
			Roguera.codeOfMenu = 1;
			try {
				GUIWindow.close();
	...
	private static void ConstructMenuPanel() {
		...
		if (SaveLoadSystem.saveFileExists()) {
			log.info("[MAIN_MENU]Save file was found");
			basePanel.addComponent(new Button("Continue game from last save", () -> {
				try {
					...
					log.info(Colors.RED_BRIGHT + "[ERROR][MAIN_MENU]: Saving file has been obsoleted.");
					e.printStackTrace();
					CheckErrorCode(CORRUPTED_SAVE_FILE);
				}
			}));
		} else {
			log.info("[MAIN_MENU]Save files was not found");
		}

		if (isAnyAuthExists()) {
			log.info("[MAIN_MENU] Auth file(s) is found");
			basePanel.addComponent(new Button("Continue game with exist character",
					MainMenu::OpenCharacterChooseWindow));
		} else {
			log.info("[MAIN_MENU] Auth files is not found");
		}
