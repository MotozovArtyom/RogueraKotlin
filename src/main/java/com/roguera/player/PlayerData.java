...
public class PlayerData implements Serializable {
	...
	public void recountStats(String stat, int amount) {
		try {
			...
			log.info("[REFLECTION] base stat " + baseStat.getInt(this) + " _statPotionBonus " + _statPotion + " amount " + amount);

			this.getClass().getDeclaredField("_" + stat).setInt(this, amount);

		} catch (NoSuchFieldException | IllegalAccessException e) {
			log.info("[ERROR][REFLECTION]No such field: " + "_" + stat);
		}
	}
	...
	public void setScore(int score) {
		if (checkScoreHash()) {
			//log.info("[PLAYER_DATA]"+Colors.GREEN_BRIGHT+"Score is not compromised!");
			this.score += score * scoreMultiplier;
			updateHash();
			//JDBСQueries.updateUserScore();
		} else {
			//log.info("[PLAYER_DATA]"+Colors.RED_BRIGHT+"Score is compromised!");
			this.score = 0;
			updateHash();
		}
