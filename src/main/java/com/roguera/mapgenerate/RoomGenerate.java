...
import com.roguera.Main;
import com.roguera.creatures.NPC;
import com.roguera.gamelogic.MobFactory;
import com.roguera.gamelogic.NPCLogicFactory;
...
public class RoomGenerate implements Serializable {
	...
	private static final Logger log = LoggerFactory.getLogger(RoomGenerate.class);
	...
	public static void generateRoomSequence(int floorNumber) {
		...
		log.info("[ROOM_GENERATE] Generate room sequence on floor " + Dungeon.currentFloorNumber);
		...
		if (!Main.isNewGame()) {
			...
			log.info("[DUNGEON] new floor number by size: " + floorNumber);
		}
	...
	public static void generateRoomStructure(Room room) {
		...
		log.info("[GENERATE_ROOM] Generate room: " + room.roomNumber + "(floor " + room.floorNumber + ")");
